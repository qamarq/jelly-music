...
internal const val JELLYFIN_SETUP_ROUTE = "jellyfin_setup"
...
internal const val JELLYFIN_LIBRARY_ROUTE = "jellyfin_library"
...
private fun Bundle?.getSafeLong(key: String): Long? {
...
internal fun androidx.navigation.NavBackStackEntry.elovaireConcreteRoute(): String? {
    return when (destination.route) {
        "$ALBUM_ROUTE/{albumId}" -> arguments.getSafeLong("albumId")?.let { "$ALBUM_ROUTE/$it" }
        "$ALBUM_TAG_EDITOR_ROUTE/{albumId}" -> arguments.getSafeLong("albumId")?.let { "$ALBUM_TAG_EDITOR_ROUTE/$it" }
        "$PLAYLIST_ROUTE/{playlistId}" -> arguments.getSafeLong("playlistId")?.let { "$PLAYLIST_ROUTE/$it" }
        "$GENRE_ROUTE/{genre}" -> arguments?.getString("genre")?.let { "$GENRE_ROUTE/${Uri.encode(it)}" }
        "$ARTIST_ROUTE/{artistName}" -> arguments?.getString("artistName")?.let { "$ARTIST_ROUTE/${Uri.encode(it)}" }
        "$LIBRARY_COLLECTION_ROUTE/{kind}" -> arguments?.getString("kind")?.let { "$LIBRARY_COLLECTION_ROUTE/$it" }
...
internal fun androidx.navigation.NavBackStackEntry.concreteNavigationRoute(): String? {
    ...
    val args = arguments
    return when (route) {
        "$ALBUM_ROUTE/{albumId}" -> args.getSafeLong("albumId")?.let { "$ALBUM_ROUTE/$it" }
        "$ALBUM_TAG_EDITOR_ROUTE/{albumId}" -> args.getSafeLong("albumId")?.let { "$ALBUM_TAG_EDITOR_ROUTE/$it" }
        "$PLAYLIST_ROUTE/{playlistId}" -> args.getSafeLong("playlistId")?.let { "$PLAYLIST_ROUTE/$it" }
